...
FileName = 'SimulationResults1';
N_Simulations = 1000;               % Discretization steps of the emprically computed mean and variance signals
N_Trajectories = 100;                % Number of Trajectories used to compute statistics (transient)
N_MaxRxnEvents = 1e6;               % Maximum Number of Events (used for N_Trajectory Simulation up to time = tf)
Iterations = 100;                  	% Number of Outer Iterations for the batch stochastic Simulations: Each Iteration runs N_Trajectories using parfor (In paper: Iterations = 512)
...
for iter = 1 : Iterations
    iter
    T_Full = cell(N_Trajectories,1);
    X_Full = cell(N_Trajectories,1);
    Y_Full = cell(N_Trajectories,1);
    parfor i = 1 : N_Trajectories
        [T_Full{i}, X_Full{i}] = SSA(Prop_ClosedLoop, S_ClosedLoop, Parameters_ClosedLoop, IC_Full, tf, N_MaxRxnEvents);
        Y_Full{i} = X_Full{i}(Parameters_ClosedLoop.Plant.L,:);
        FinalDistribution(iter,i) = Y_Full{i}(end);
    end
    [time_vector, MeanY_Iteration(iter,:), VarY_Iteration(iter,:)] = ComputeStatistics(T_Full, Y_Full, N_Simulations);
%     if iter < Iterations
        clear T_Full X_Full Y_Full
%     end
end
Mean_Full = mean(MeanY_Iteration, 1);
Variance_Full = mean(VarY_Iteration,1) + mean(MeanY_Iteration.^2,1) - Mean_Full.^2;
FinalDistribution_Full = FinalDistribution(:);
SimTime_Full = toc(TStart)
...
for iter = 1 : Iterations
    iter
    T_Reduced = cell(N_Trajectories,1);
    X_Reduced = cell(N_Trajectories,1);
    Y_Reduced = cell(N_Trajectories,1);
    parfor i = 1 : N_Trajectories
        [T_Reduced{i}, X_Reduced{i}] = SSA(Prop_ReducedClosedLoop, S_ReducedClosedLoop, Parameters_ReducedClosedLoop, IC_Reduced, tf, N_MaxRxnEvents);
        Y_Reduced{i} = X_Reduced{i}(Parameters_ReducedClosedLoop.Plant.L,:);
        FinalDistribution(iter,i) = Y_Reduced{i}(end);
    end
    [time_vector, MeanY_Iteration(iter,:), VarY_Iteration(iter,:)] = ComputeStatistics(T_Reduced, Y_Reduced, N_Simulations);
%     if iter < Iterations
        clear T_Reduced X_Reduced Y_Reduced
%     end
end
Mean_Reduced = mean(MeanY_Iteration, 1);
Variance_Reduced = mean(VarY_Iteration,1) + mean(MeanY_Iteration.^2,1) - Mean_Reduced.^2;
FinalDistribution_Reduced = FinalDistribution(:);
SimTime_Reduced = toc(TStart)
