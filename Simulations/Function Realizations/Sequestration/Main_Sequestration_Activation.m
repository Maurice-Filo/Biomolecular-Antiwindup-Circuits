...
f_eta = @(u, eta)( (Parameters.k/2) * ((u - Parameters.u_0)/Parameters.delta_2 - Parameters.delta_1/eta + ...
                                      sqrt( ((u - Parameters.u_0)/Parameters.delta_2 - Parameters.delta_1/eta).^2 + ...
                                      4*Parameters.delta_1*u / Parameters.delta_2 / eta)) );
f_infinity = @(u)( (Parameters.k/Parameters.delta_2) * max( (u - Parameters.u_0), 0 ) ); 
...
Handle_Axis1 = axes(Handle_Figure1);
    ...
    Handle_Axis1.YLim = [0, y_eta(end)];
for i = 1 : length(u_max_vector)
    plot(Handle_Axis1, t{i}, Parameters.k * V{i}(2,:), 'Color', Colors(i,:), 'LineWidth', LineWidth_Thick);
end
...
Handle_Axis4 = axes(Handle_Figure4);
    ...
    Handle_Axis4.XLim = [0, 5];
    Handle_Axis4.YLim = [0, 3];
...
if Save_Flag == 1  
    ...
    print(Handle_Figure1, 'Output_Seq_Act', '-dpdf', '-painters');
    Handle_Figure1.Color = [1, 1, 1];
    
    Handle_Figure2.Color = 'none';
    set(Handle_Figure2, 'InvertHardCopy', 'off');
    print(Handle_Figure2, 'Input_Seq_Act', '-dpdf', '-painters');
    Handle_Figure2.Color = [1, 1, 1];
    
    Handle_Figure3.Color = 'none';
    set(Handle_Figure3, 'InvertHardCopy', 'off');
    print(Handle_Figure3, 'Map_Seq_Act', '-dpdf', '-painters');
    Handle_Figure3.Color = [1, 1, 1];
    
    Handle_Figure4.Color = 'none';
    set(Handle_Figure4, 'InvertHardCopy', 'off');
    print(Handle_Figure4, 'Convergence_Seq_Act', '-dpdf', '-painters');
    Handle_Figure4.Color = [1, 1, 1];
end
